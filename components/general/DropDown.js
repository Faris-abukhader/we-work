...
export const DropDown = ({selectedItem='',selectedId='',isReview=false,hasLabel,mainLabel='',label,isValid=true,py=2,hasData=false,data=[],hasHandler=true,handler})=> {
...
<div>
  ...
  {hasData && <div className={`${showList ? 'block':'hidden'} origin-top-right absolute right-0 mt-2 w-full h-44  overflow-scroll p-1 rounded-md ring-0.7 ring-gray-300 bg-white  z-20 focus:outline-none"`}>
    {showList && data.map((item,index)=>{return <div key={index} onClick={()=>clickHandler(item.id,item.title)} className={` px-3 py-1 w-full hover:bg-gray-200 hover:text-white rounded-lg cursor-pointer`} >{item.title} </div>})}
  </div>
  }
  </div>
  )
