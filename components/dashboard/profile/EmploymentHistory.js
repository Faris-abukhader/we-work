...
export default function EmploymentHistory() {
    ...
    return (
        <div className='p-4 mt-5 border-2 rounded-2xl'>
            ...
            {employmentHistory && employmentHistory.map((employment) => <div className='text-gray-600 flex items-center justify-between py-6' key={employment.id}>
                ...
                <div className='flex space-x-3'>
                    <ModifyButton onClick={toggleEditModel} id={employment.id} />
                    <DeleteButton onClick={deleteOneEmploymentHistory} id={employment.id} />
                </div>

            </div>)}
            <AddEmploymentModel show={showAddModel} toggle={toggleAddModel} />
            <UpdateEmploymentModel show={showEditModel} toggle={toggleEditModel} data={editModelData} />
        </div>
    )
}
